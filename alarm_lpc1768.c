...
int main(void)
{
  ...
  LPC_GPIO0->FIODIR |= 0x0F << 23 | 1 << 27 | 1 << 28;
	...
	if (count == 4) // Check if all four digits have been entered
  {
		if (alarm_hour > 23 || alarm_min > 59) // Check if hour or minute values are out of range
    {
...
void rtc_init()
{
   ...
   LPC_RTC->DOM    = 02;   // Update date value 
   LPC_RTC->MONTH  = 04;   // Update month value
   LPC_RTC->YEAR   = 2024; // Update year value

   // Set Time 10:40:25 AM 
   LPC_RTC->HOUR   = 11;   // Update hour value 
   LPC_RTC->MIN    = 10;   // Update min value
   LPC_RTC->SEC    = 35;   // Update sec value 
}
//sound the buzzer
void sound_Buzzer()
{
	if(buz_flag==1)
	{
		if(buz_cond%2==0)
		{
			LPC_GPIO0->FIOSET=0x800; //buzzer on //buzzer off
			buz_cond++;
		}
		if(buz_cond%2!=0)
		{
			LPC_GPIO0->FIOCLR=0x800;
			buz_cond++;
		}
		if(buz_cond==20)
			buz_flag=0;
